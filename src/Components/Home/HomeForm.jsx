...
const HomeForm = () => {
	...
	const [errorMessage, setErrorMessage] = React.useState(
		'Debes completar todos los campos para modificar el Slide'
	)
	...
	const handleTextChange = (id) => (e) => {
		...
		setSlides((slides) =>
			slides.map((slide) =>
				slide.id === id ? { ...slide, text: value, error: false } : slide
			)
		)
	}
	...
	const handleImageChange = (id) => (e) => {
		...
		setSlides((slides) =>
			slides.map((slide) =>
				slide.id === id ? { ...slide, image: value, error: false } : slide
			)
		)
	}
	...
	const handleWelcomeSubmit = (e) => {
		if (welcome.length > 20) {
			...
			setErrorMessage(
				'El mensaje de bienvenida debe tener al menos 20 caracteres'
			)
		}
	}
